...
from data_split.iemocap_split import audio_partition
...
if __name__ == "__main__":
    ...
    if not Path(args.raw_data_path).exists(): 
        raise Exception("IEMOCAP data not found, please check arg raw_data_path!")
    if args.test_session not in ['Session1', 'Session2', 'Session3', 'Session4', 'Session5']:
        raise Exception("Invailid test session, available options: Session1; Session2; Session3; Session4; Session5.")
    ...
    if fl_feature == True:
        save_file_name = (
            args.setup+"_dataset_" + args.process_method + "_" + args.feature_type + "_"+ str(args.test_session) + "_db" + str(args.db_level) + ".p"
        )
    else:
        save_file_name = (
            args.setup + "_dataset_" + args.process_method + "_" + args.feature_type + "_" + str(args.test_session) + ".p"
        )
